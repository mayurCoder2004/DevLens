...
const engineeringHealthService = {
  ...
  const engineeringScore =
    this.calculateEngineeringScore(repository);

  const status =
    this.getEngineeringStatus(engineeringScore);
  ...
  if (data.architecture) {
    const architectureScore =
      this.calculateArchitectureHealthScore(
        data.architecture
      );

    totalScore +=
      architectureScore *
      ENGINEERING_WEIGHTS.architecture;
  ...
  if (data.deployment) {
    totalScore +=
      data.deployment.deploymentScore *
      ENGINEERING_WEIGHTS.deployment;
  ...
  if (data.pullRequestAnalyses.length > 0) {
    ...
    totalScore +=
      pullRequestScore *
      ENGINEERING_WEIGHTS.pullRequest;
  ...
  scores: {
    ...
    technicalDebt:
      repository.technicalDebt?.technicalDebtScore ?? null,

    deployment:
      repository.deployment?.deploymentScore ?? null,
...
collectPullRequestStrengths(repository) {
  ...
  const latestPullRequest =
    repository.pullRequestAnalyses[0] ?? null;
